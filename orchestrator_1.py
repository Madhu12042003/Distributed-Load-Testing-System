...
def start_load_test(test_id, test_type, test_message_delay,node_id):
    if node_id in driver_nodes:
...
def kafka_listener():
    for message in consumer1:
        if message.topic == topic1:
            ...
            configure_test(message.value['test_id'], message.value['test_type'], message.value['test_message_delay'],message.value['node_id'])
